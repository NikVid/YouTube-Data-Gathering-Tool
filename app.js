...
app.get("/download:keyword", async function(req, res) {
  ...
  for (let y = 0; y < tokenList.length; y++) {
    ...
    for (let x = 0; x < videoIDs[y].length; x++) {
      finalFile.push([loadedPages[y][0][x]?.snippet?.title || "NaN", loadedPages[y][0][x]?.snippet?.thumbnails?.high?.url || "NaN", loadedPages[y][1][x]?.snippet?.title || "NaN", loadedPages[y][0][x]?.snippet?.description || "NaN", loadedPages[y][0][x]?.snippet.publishedAt || "NaN", videoUrl + videoIDs[y][x], loadedPages[y][0][x]?.statistics.viewCount || "NaN", loadedPages[y][0][x]?.statistics.likeCount || "NaN", loadedPages[y][0][x]?.statistics?.commentCount || "disabled", loadedPages[y][1][x]?.snippet?.description || "NaN",loadedPages[y][1][x]?.statistics?.subscriberCount || "NaN", loadedPages[y][0][x]?.topicDetails?.topicCategories?.join(",") || "NaN", loadedPages[y][0][x]?.snippet.categoryId || "NaN", loadedPages[y][1][x]?.snippet.country || "NaN", loadedPages[y][0][x]?.snippet?.defaultAudioLanguage || "NaN", loadedPages[y][0][x]?.snippet?.tags?.join(",") || "NaN", loadedPages[y][0][x]?.contentDetails?.duration || "NaN"]);
    }
  }
